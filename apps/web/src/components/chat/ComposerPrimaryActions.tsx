...
export const ComposerPrimaryActions = memo(function ComposerPrimaryActions({
  ...
  if (isRunning) {
    // Steering: a mid-run send adds the message to the running turn, so the
    // send affordance stays available alongside stop whenever there is
    // content to send.
    return (
      <div className={cn("flex items-center justify-end", compact ? "gap-1.5" : "gap-2")}>
        {hasSendableContent ? (
          <button
            type="submit"
            className="flex size-8 cursor-pointer items-center justify-center rounded-full bg-primary/90 text-primary-foreground shadow-xs shadow-primary/24 inset-shadow-[0_1px_--theme(--color-white/16%)] transition-all duration-150 hover:bg-primary hover:scale-105 active:inset-shadow-[0_1px_--theme(--color-black/8%)] active:shadow-none disabled:pointer-events-none disabled:opacity-30 disabled:shadow-none sm:h-8 sm:w-8"
            {...pointerFocusProps}
            disabled={isSendBusy || isSendDisabled || isConnecting || isEnvironmentUnavailable}
            aria-label="Send message to steer"
          >
            {isSendBusy ? (
