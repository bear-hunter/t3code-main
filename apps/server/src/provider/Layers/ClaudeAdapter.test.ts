...
describe("ClaudeAdapterLive", () => {
  ...
  it.effect(
    "treats diagnostic-only error results from newer CLIs as interrupted without a runtime error",
    () => {
      const harness = makeHarness();
      return Effect.gen(function* () {
        const adapter = yield* ClaudeAdapter;
        ...
        // Claude Code >= 2.1.x reports a user cancel as an error result whose
        // only error line is internal "[ede_diagnostic]" telemetry.
        harness.query.emit({
          type: "result",
          subtype: "error_during_execution",
          is_error: true,
